...
from rascube_v2 import AddonId, AddonUnavailableError, AsyncRASCube, prompt_connection
...
async def main(port: str, serial_number: int) -> None:
    ...
    async with AsyncRASCube(port, serial_number=serial_number) as cube:
        ...
        try:
            async with wheel.samples.subscribe() as samples:
                while plt.fignum_exists(figure.number):
                    try:
                        sample = await asyncio.wait_for(anext(samples), timeout=POLL_SECONDS)
                    except TimeoutError:
                        sample = None
